  ...
  nav_panel(
    ...
    layout_columns(
      ...
      card(card_header("Methods & mathematics"), tags$div(class = "m-body",
        ...
        # ----------------------------------------------------------- sethi
        .h("6. Sethi et al. (2016) error-tolerant match calling", id = "m-sethi",
           tag = "recommended", tagclass = "rec"),
        .p(class = "m-lead",
          "Same likelihood machinery as method 5, with two deliberate differences that ",
        ...
        .p("Rather than asking you to name the alternative, it computes the likelihood under ",
           "every relationship you are willing to consider and divides by whichever explains ",
           "the pair best:"),
        .eq("\\Lambda \\;=\\; \\frac{L(\\text{same individual} \\mid G_1, G_2)}{\\max\\left\\{ L(\\text{unrelated}),\\; L(\\text{full sib}),\\; L(\\text{parent--offspring}) \\right\\}}"),
        .p("The maximum is taken over the ", tags$b("whole multilocus genotype"), ", not locus ",
           "by locus. A pair is called a match only if one animal beats every competing ",
           "relationship, which is conservative in the right direction and removes the ",
           "“which alternative do I assume?” decision entirely."),
        .p("It also tells you something useful for free: the app reports which relationship ",
           "came closest for each matched pair. If one dominates, that is the alternative your ",
           "other analyses should be using."),
        ...
        .p("The paper's rule is simply \\(\\Lambda > 1\\): the match hypothesis has to be ",
           ...
           "nothing to tune."),
        .p("That is a real advantage — there is no prior to argue about — and a real ",
           "limitation, because it takes no account of how many pairs you are testing. With ",
           "\\(n\\) samples you evaluate \\(\\binom{n}{2}\\) pairs, and a rule with no ",
        ...
        .s("How the two compare in practice"),
        .p("Because \\(\\Lambda\\) and method 5's likelihood ratio share their numerator ",
           "and, whenever full siblings is the closest competing relationship, their ",
           "denominator too, the two often produce ", tags$i("numerically identical"),
           " evidence and differ only in where they cut. On the example dataset shipped with ",
           "this app, the Sethi rule recovers the known truth exactly; on a larger dataset it ",
           "merges more than the posterior rule. Neither is universally right, and the honest ",
           "reporting is the range they bracket."),
        .p("Their clustering step — compare each sample against all others, merge on a ",
           "match, repeat until memberships stop changing — is exactly the connected ",
           "components of the match graph, which is what the single-linkage option below does."),
        ...
        tags$div(class = "m-def", style = "font-size:.88rem;",
          ...
          .p("Wang, J. (2004). Sibship reconstruction from genetic data with typing errors. ",
             tags$i("Genetics"), " 166, 1963\u20131979."),
